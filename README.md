...
Material UI (MUI) – Used for input fields, buttons, cards, and icons
...
The frontend sends a request to the OpenWeather API.

The API returns weather data in JSON format.

The app checks whether the response is valid.

If the city exists:

Weather details are extracted.

The UI updates automatically using React state.
...
How fetch works and how to handle API errors manually
...
API key is currently used on the frontend
...
Support more weather conditions (cloudy, mist, snow)

Improve mobile responsiveness

Convert API handling to Axios
